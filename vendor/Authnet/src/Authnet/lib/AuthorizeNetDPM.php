...
class AuthorizeNetDPM extends AuthorizeNetSIM {
     ...
     * purposes.
     */
    public static function directPostDemo($url, $api_login_id, $transaction_key, $amount = "0.00", $md5_setting = "", $environment) {
        

        // Step 1: Show checkout form to customer.
        if (!count($_POST) && !count($_GET)) {           
            $fp_sequence = time(); // Any sequential number like an invoice number.
            return AuthorizeNetDPM::getCreditCardForm($amount, $fp_sequence, $url, $api_login_id, $transaction_key, $environment);
        } 
        ...
        elseif (count($_POST)) {
            ...
            if (!$response->isAuthorizeNet()) {
                if (!$response->approved) {
                    ...
                    $url = '/student_enrollment/public/transaction/authnet/receipt';
     ...
     * @return string
     */
    public static function getCreditCardForm($amount, $fp_sequence, $relay_response_url, $api_login_id, $transaction_key, $environment, $prefill = true) {
        $time = time();
